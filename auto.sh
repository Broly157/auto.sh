...
prompt_confirm() {
  while true; do
    read -r -n 1 -p "${1:-Continue?} [y/n]: " REPLY
    case $REPLY in
      [yY]) printf "\nStarted 400 BYpass Scanning\n" && cat 400.txt | while read i ; do byp455 $i | egrep curl | sort -u | tee 400; done && if [ ! -s 400.txt ]; then echo "The tool worked but didn't Found Any Bypass" >> 400.txt;else echo "You got some bypass in 400.txt";fi ; return 0
        ;;
      [nN]) echo ; return 1 ;;
      *) printf " \033[31m %s \n\033[0m" "Bruh..Only {Y/N}"
    esac
